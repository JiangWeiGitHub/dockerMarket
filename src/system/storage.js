...
const volumeMountpoint = vol => '/run/dockerMarket/volumes/' + vol.uuid
const blockMountpoint = blk => '/run/dockerMarket/blocks/' + blk.name
...
const probeUsages = async mounts => {
  ...
  let filtered = mounts.filter(mnt => mnt.fs_type === 'btrfs' && 
    mnt.mountpoint.startsWith('/run/dockerMarket/volumes/') && !mnt.mountpoint.endsWith('/graph/btrfs'))
