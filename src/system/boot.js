  ...
  await Promise
    ...
    .each((inspection, index) => {
      if (inspection.isFulfilled())
        mps[index].ref.dockerMarket = inspection.value() 
      else {
        console.log(inspection.reason())
        mps[index].ref.dockerMarket = 'ERROR'
      }
    })
...
const probeAllAsync = async fileSystems =>
  Promise.map(fileSystems.filter(shouldProbeFileSystem),
    async fsys => {
      try {
        fsys.dockerMarket = await fruitmix.probeAsync(fsys.mountpoint)
      }
      catch (e) {
        fsys.dockerMarket = { status: 'EFAIL' }
      }
    })
...
const assertReadyToBoot = dockerMarket =>
  (!dockerMarket || typeof dockerMarket !== 'object' || dockerMarket.status !== 'READY')
    ? throwError('fruitmix status not READY')
    : true

const assertReadyToInstall = dockerMarket =>
  (!dockerMarket || typeof dockerMarket !== 'object' || dockerMarket.status !== 'ENOENT')
...
module.exports = {
  ...
  autoBootAsync: async function() {
    ...
    if (last) {    
      ...
      if (fsys) {
        try {
          ...
          assertReadyToBoot(fsys.dockerMarket)
          this.boot(cfs(fsys))
        }
        catch (e) {
    ...
    let alts = fileSystems.filter(f => {
      try { 
        ...
        assertReadyToBoot(f.dockerMarket)
        return true
      } 
      catch(e) {
        return false
  ...
  manualBootAsync: async function (args) {
    ...
    assertFileSystemGood(fsys)
    let dockerMarket = await fruitmix.probeAsync(fsys.mountpoint)

    if (reinstall === true || install === true) {
      if (reinstall) {
        await rimrafAsync(path.join(fsys.mountpoint, 'dockerMarket'))
        await mkdirpAsync(path.join(fsys.mountpoint, 'dockerMarket', 'fruitmix'))
      }
      else {
        assertReadyToInstall(dockerMarket)
      }
    }
    else { // direct boot, fruitmix status must be 'READY'
      assertReadyToBoot(dockerMarket) 
    }
