...
// ip addr add ${ipaddr}/24 dev ${dev} label ${dev}:dockerMarket
// ip addr del ${ipaddr}/24 dev ${dev}:dockerMarket
...
const _deleteAlias = (dev, addr, callback) => 
  child.exec(`ip addr del ${addr}/24 dev ${dev}:dockerMarket`, err => callback(err))
