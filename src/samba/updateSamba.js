...
const addSmbUsersAsync = async (fusers) => {
  ...
  await mkdirpAsync('/run/dockerMarket/smb')
  await fs.writeFileAsync('/run/dockerMarket/smb/tmp', text)
  await child.execAsync('pdbedit -i smbpasswd:/run/dockerMarket/smb/tmp')
